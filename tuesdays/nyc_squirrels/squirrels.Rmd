...
```{r}
behavior<-nyc_squirrels %>% group_by(shift) %>%  summarize(Running=sum(running, na.rm=T),Eating=sum(eating, na.rm=T),Chasing=sum(chasing, na.rm=T), Climbing=sum(climbing, na.rm=T),Foraging=sum(foraging, na.rm=T) )
...
```{r}
# parameters
#pdf('squirrel_behavoir.pdf',height=5, width=5)
circos.clear()
...
circos.trackPlotRegion(
  ...
  panel.fun = function(x, y) {
    ...
    circos.axis(
      ...
      minor.ticks = 1, 
      major.tick.percentage = 0.5,labels.cex = 0.7,
